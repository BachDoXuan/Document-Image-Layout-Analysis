...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 13:37:34 2019
...
import numpy as np
from dh_segment import utils
from tqdm import tqdm
from typing import Union, List
from enum import Enum
import pandas as pd
from dh_segment.io.input_utils import data_augmentation_fn, \
     extract_patches_fn, load_and_resize_image, rotate_crop, \
     resize_image, local_entropy
...
def input_fn(input_data: Union[str, List[str]], params: dict, 
             input_label_dir: str=None, data_augmentation: bool=False, 
             batch_size: int=5, make_patches: bool=False, num_epochs: int=1,
             num_threads: int=4, image_summaries: bool=False):
    """
    Input_fn for estimator
    
    :param input_data: input data. It can be a directory containing the images, 
        it can be
    ...
    :param data_augmentation: boolean, if True will scale, rotate, ... 
        the images
    :param batch_size: size of the batch
    :param make_patches: bool, whether to make patches 
        (crop image in smaller pieces) or not
    :param num_epochs: number of epochs to cycle through data 
        (set it to None for infinite repeat)
    :param num_threads: number of thread to use in parallel 
        when using tf.data.Dataset.map
    :param image_summaries: boolean, whether to make tf.Summary to watch on 
        tensorboard
    :return: fn
    """
    ...
    # --- Map functions
    def _make_patches_fn(input_image: tf.Tensor, label_image: tf.Tensor, 
                         offsets: tuple) -> (tf.Tensor, tf.Tensor):
        with tf.name_scope('patching'):
            patches_image = extract_patches_fn(input_image, 
                                               training_params.patch_shape, 
                                               offsets)
            patches_label = extract_patches_fn(label_image, 
                                               training_params.patch_shape, 
                                               offsets)
    ...
    def _load_image_fn(image_filename, label_filename):
        if (training_params.data_augmentation and 
            training_params.input_resized_size > 0):
